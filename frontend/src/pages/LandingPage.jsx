...
const DestinationCard = ({ city, country, tag, img }) => {
    return (
        <motion.div
            ...
            className="relative flex-shrink-0 w-[230px] h-[310px] rounded-[28px] overflow-hidden group cursor-pointer shadow-md"
        >
            ...
            <div className="absolute bottom-7 left-6">
                <span className="text-[9px] font-extrabold tracking-[0.22em] text-white/60 uppercase mb-1.5 block">{tag}</span>
                <h3 className="text-[22px] font-bold text-white mb-0.5 leading-tight">{city}</h3>
                <p className="text-white/55 text-[13px] font-medium">{country}</p>
            </div>
        </motion.div>
    );
};
...
const LandingPage = () => {
    ...
    return (
        <div className="min-h-screen font-sans flex flex-col relative w-full overflow-hidden"
            ...
            >
                <div className="absolute inset-0 bg-[#FAF9F4]/80"></div> {/* overlay to fade it beautifully */}
            </div>
            ...
            {/* Navbar */}
            <header className="relative z-10 flex items-center justify-between px-6 pt-8 pb-6 w-full max-w-7xl mx-auto">
                <div className="flex items-center gap-2 font-extrabold text-[22px] text-[#222222] tracking-tight cursor-pointer">
            ...
            {/* Main Hero Content */}
            <main className="relative z-10 flex flex-col items-center justify-center flex-1 px-4 text-center mt-20 mb-32">
                <div className="mb-8 flex items-center gap-2">
                    <div className="inline-block px-4 py-1.5 rounded-full border border-[#D5D8CB] bg-[#F1F3EA] text-[11px] font-extrabold tracking-[0.2em] text-[#6D8365] uppercase">
                        <span className="inline-block w-1.5 h-1.5 bg-[#6D8365] rounded-full mr-2 mb-[1px]"></span>
                        AI-POWERED ITINERARIES IN SECONDS
                    </div>
                </div>

                <h1 className="text-5xl md:text-7xl font-[800] tracking-tight text-[#222222] max-w-4xl leading-[1.05] mb-6">
                    Plan the trip of <br className="hidden md:block" /> your <span className="text-[#222222] relative inline-block">
                        daydreams<span className="text-[#222222]">.</span>
                        <span className="absolute bottom-1 left-0 w-full h-[6px] bg-[#6D8365]/30"></span>
                    </span>
                </h1>

                <p className="text-[17px] md:text-[20px] text-[#5C5C5C] max-w-2xl mx-auto font-medium leading-relaxed mb-12">
                ...
                <div className="w-full max-w-3xl bg-white rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.06)] p-2.5 flex items-center mb-10 border border-gray-100">
                    <div className="pl-4 pr-3">
                        <Search className="text-gray-400" size={26} strokeWidth={2.5} />
                    </div>
                    <input
                        type="text"
                        className="flex-1 bg-transparent border-none outline-none text-gray-800 placeholder-gray-400 text-lg py-3 font-medium"
                    ...
                    <button
                        ...
                        className="bg-[#6D8365] hover:bg-[#586A51] text-white px-8 py-4 rounded-full text-[15px] font-bold transition-colors shadow-sm ml-2"
                    >
                        Plan my trip
                    </button>
                </div>
                ...
                <div className="flex flex-wrap justify-center gap-3 max-w-3xl">
                    {["Paris weekends", "Island hopping", "Slow trains in Europe", "Cherry blossoms"].map((tag) => (
                        <button
                            key={tag}
                            className="px-5 py-2.5 bg-white hover:bg-gray-50 border border-gray-200 rounded-full text-[14px] font-bold text-[#5C5C5C] transition-colors shadow-sm"
            ...
            <section className="relative z-10 pb-24 w-full">
                <div className="max-w-3xl mx-auto px-4 mb-9 text-center">
                    <span className="text-[11px] font-extrabold tracking-[0.28em] text-[#C2723A] uppercase block mb-2">
                        Trending right now
                    </span>
                    <h2 className="text-4xl md:text-5xl font-extrabold text-[#222222] tracking-tight">
                        Where wanderers are{' '}
                        <span className="italic font-serif font-light text-[#6D8365]">drifting</span>
                    </h2>
                </div>
                <div className="flex justify-center gap-5 overflow-x-auto pb-4 px-6 no-scrollbar">
                    {destinations.map(d => <DestinationCard key={d.city} {...d} />)}
                </div>
            </section>

            {/* Testimonial Section */}
            <section className="relative z-10 py-32 bg-white/50 border-y border-gray-100">
                <div className="max-w-4xl mx-auto px-6 text-center">
                    <span className="text-4xl text-[#6D8365] font-serif mb-8 block opacity-40">“</span>
                    <h2 className="text-3xl md:text-4xl font-bold text-[#222222] leading-tight mb-12">
                    ...
                    <div className="flex flex-col items-center">
                        <img src="https://i.pravatar.cc/150?u=mira" alt="Mira" className="w-14 h-14 rounded-full mb-4 border-2 border-white shadow-md" />
            ...
            {/* Elegant Footer */}
            <footer className="relative z-10 py-12 px-10 max-w-7xl mx-auto w-full">
                <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-gray-200/60 gap-6">
                    <p className="text-[14px] text-gray-500 font-medium">
                        © 2026 Wanderly. Wander gently.
                    </p>
                    <p className="text-[14px] italic font-serif text-gray-400">
